<?php
	$pageTitle = "Resources";
	$metaDescription = "";
	include("header.php");
?>

<div style="margin-top:50px;">
	<h2 style="font-weight:500;text-align:center;">Resources for Ecommerce Growth</h2>

	<section id="resource-types">
		<ul>
			<li><a href="#email-course">Courses</a></li>
			<li><a href="#ebooks">EBooks</a></li>
			<li><a href="#presenation">Presentations</a></li>
			<li><a href="#podcasts">Podcasts</a></li>
			<li><a href="#blogroll">Blog</a></li>
		</ul>
	</section>

	<section id="email-course">
		<div class="clearfix">
			<h3>Free 5-Part Email course</h3>
			<div class="how">
				<h4>How Abandoned Cart Email Could Change Your Business</h4>
				
				<p>67% of your checkout sessions are abandoned. Learn how to convert up to 15% of those abandoned carts into customers with this actionable, 100% free, 5-part email course.</p>		
			</div>
			<div class="signup">
				<p>Sign Up For Free</p>
				
				<input type="text" placeholder="contactme@email.com">
				
				<a id="seehow" href="#">Yes, Start Free Course</a>
			<div>
			
		</div>
	</section>



	<section id="webinar-quote">
		<p class="quote">"Finding revenue in websites is our business, and Rejoiner has become one of our primary tactics to accomplish that."</p>
	
		<p><b>BRIAN MASSEY</b>, CONVERSION SCIENTIST &amp; AUTHOR OF <i>YOUR CUSTOMER CREATION EQUATION</i></p>
	
		<img src="img/down-arrow-line.png">
	</section>
	
	<section id="ebooks" class="clearfix">
		<h3>Our Best Ebooks</h3>
		
		<div>
			<img src="img/equation.jpg">
			<h3>The Cart Abandonment Equation</h3>
			
			<p>Cart abandonment email primer written specifically for eCommerce professionals.</p>
			
			<p><a href="#">FREE DOWNLOAD</a></p>
		</div>
		<div>
			<img src="img/lookbook.jpg">
			<h3>2013 Cart Abandoment Lookbook</h3>
			
			<p>A study of cart abandonment email strategies of the Internet Retailer 500.</p>
			
			<p><a href="#">FREE DOWNLOAD</a></p>
		</div>
		<div>
			<img src="img/remarketing.jpg">
			<h3>Email Remarketing &amp; CAN-SPAM</h3>
			
			<p>How to send CAN-SPAM compliant cart abandonment email campaigns.</p>
			
			<p><a href="#">FREE DOWNLOAD</a></p>
		</div>
		
		<div>
			<img src="img/peakdesign.jpg">
			<h3>Peak Design Case Study</h3>
			
			<p>How Peak Design recovers 12% of abandoned carts using email remarketing.</p>
			
			<p><a href="#">FREE DOWNLOAD</a></p>
		</div>
		
		<div>
			<img src="img/applecheckout.jpg">
			<h3>The Apple Checkout Experience</h3>
			
			<p>A fully documented review of the Apple’s eCommerce checkout experience.</p>
			
			<p><a href="#">FREE DOWNLOAD</a></p>
		</div>
		
		<div>
			<img src="img/warbyparker.jpg">
			<h3>Warby Parker Triggered Emails</h3>
			
			<p>How Warby Parker uses triggered email to create a magical customer experience.</p>
			
			<p><a href="#">FREE DOWNLOAD</a></p>
		</div>
		
		
	</section>

	<section id="presenation">
		<div class="clearfix">
			<h3>Presenations &amp; Webinars</h3>
			
			<img src="img/toptip.jpg">
			
			<div class="scrolly">
				<h4>MOST RECENT</h4>
				<ul>
					<li><a href="#">25 Entrepreneurs Tell What They Wish They’d Known Before Founding Their First Startup</a></li>
					<li><a href="#">10 Quotes About the Future of eCommerce</a></li>
					<li><a href="#">Shopping Cart Abandonment: The Psychology of Why &amp; How to Win Back Lost Sales</a></li>
					<li><a href="#">How to Track Shopping Cart Abandonment Using Google Analytics</a></li>
					<li><a href="#">25 Entrepreneurs Tell What They Wish They’d Known Before Founding Their First Startup</a></li>
					<li><a href="#">Shopping Cart Abandonment: The Psychology of Why &amp; How to Win Back Lost Sales</a></li>
					<li><a href="#">How to Track Shopping Cart Abandonment Using Google Analytics</a></li>
				</ul>
			</div>
			
		</div>
	</section>
	
	<section id="podcasts">
		<div class="clearfix">
			<h3>Our Latest Podcast</h3>
			
			<img src="img/soundcloud.png">
			
			<div class="scrolly">
				<h4>MOST RECENT</h4>
				<ul>
					<li><a href="#"><b>EPISODE #1</b><br>
									How Not to Pitch a Millionaire</a></li>
					<li><a href="#"><b>EPISODE #2</b><br>
									How Not to Pitch a Millionaire</a></li>
					<li><a href="#"><b>EPISODE #3</b><br>
									How Not to Pitch a Millionaire</a></li>
					<li><a href="#"><b>EPISODE #4</b><br>
									How Not to Pitch a Millionaire</a></li>
					<li><a href="#"><b>EPISODE #5</b><br>
									How Not to Pitch a Millionaire</a></li>
					<li><a href="#"><b>EPISODE #6</b><br>
									How Not to Pitch a Millionaire</a></li>
				</ul>
			</div>
			
		</div>
	</section>

	<section id="blogroll">
		<div class="clearfix">
			<h3>The Conversion Sense Blog</h3>
			
			<div id="latestpost">
				<h5>Our Latest Post</h5>
				<p><a href="">Google Analytics Enhanced eCommerce: Better Data, 
Better Decisions for Retailers.</a></p>

				<img src="img/conversions.png">
				
			</div>
			
			<div id="hits">
				<h5>Our Greatest Hits</h5>
				<a href="#">AdWords DIYers: When is the Right Time to Hire a Professional?</a>
				<a href="#">AdWords DIYers: When is the Right Time to Hire a Professional?</a>
				<a href="#">AdWords DIYers: When is the Right Time to Hire a Professional?</a>
				<a href="#">AdWords DIYers: When is the Right Time to Hire a Professional?</a>
				
				<h5 id="join">Join 3,000+ Subscribers</h5>
				<input type="text" placeholder="contactme@email.com">
				
				<a id="seehow" href="#">Subscribe</a>
				
			</div>
			
			
			
		</div>
	</section>
</div>

			
<?php include("footer.php"); ?>