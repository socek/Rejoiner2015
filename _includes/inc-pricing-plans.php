
	<div class="pricing-notes">
		<h3>All of Our Packages Include:</h3>
		<p>Full service account management. Unlimited email sends. Unlimited email design. HTML/CSS Development. Campaign strategy. Monthly Reviews.</p>
	</div>

	<!-- start major row -->
	<div class="row pricing-table container"> 
		<div class="far-left-labels">
			<div class="packages-label"><h2>Packages</h2></div>
			<div class="campaigns-label"><h2>Campaigns</h2></div>
		</div>

		<!-- /////////////////////////////////
		start first row 
		/////////////////////////////////////
		-->
		<div class="row">
			<!-- start recover plan -->
			<div id="recoverPlan" class="three columns pricing-col">
			    <h1>RECOVER</h1>
			    <small>Reduce Cart Abandonment</small>
			    <hr>
			    <span class="label-cost">
			        $1,000<span style="color:#6d6e71;">/mo</span>
			    </span>
			    <br>
			    <small class="label-show">CAMPAIGNS</small><br>
			        <b class="mobile-campaign-title">Campaigns</b>
			        	<p>Cart Abandonment</p>
			       		<p>Browse Abandonment</p>
			        	<p>Wishlist Abandonment</p>
			    <span class="mobile-details">
				    <br>
				        <b class="mobile-campaign-title">Features</b>
					        <p>Email Analytics<p>
					        <p>Funnel Tracking</p>
					        <p>Pre-Submit Capture</p>
					        <p>Custom Email Templates</p>
					        <p>Frequency Capping</p>
				    <br>
			    </span>
			</div> <!-- end recover plan -->

			    <!-- start retain plan -->
			<div id="retainPlan" class="three columns pricing-col">
			    <h1>RETAIN</h1>
			    <small>Retain Existing Customers</small>
			    <hr>
			    <span class="label-cost">
			        $2,000<span style="color:#6d6e71;">/mo</span>
			    </span>
			    <br>
			    <small class="label-show">CAMPAIGNS</small>
			        <br>
			        <b class="mobile-campaign-title">Campaigns</b>
				        <p><b>ALL</b> Recover Campaigns</p>
				        <p class="label-plus">PLUS</p>
				        <p>Welcome</p>
				        <p>Feedback Request</p>
				        <p>Replenishment</p>
				        <p>VIP Customers</p>
				        <p>Win Back</p>
			    <span class="mobile-details">
				    <br>
				        <b class="mobile-campaign-title">Features</b>
				            <p><b>All</b> Recover Features<p>
				            <p class="label-plus">PLUS</p>
				            <p>Segmentation</p>
				    <br>
			    </span>
			</div><!-- end retain plan -->

			<!-- start optimize plan -->
			<div id="optimizePlan" class="three columns pricing-col">
			    <h1>OPTIMIZE</h1>
			    <small>Test & Optimize Campaigns</small>
			    <hr>
			    <span class="label-cost">
			    	<span style="color:#609736; font-weight:700;">+</span>$500<span style="color:#6d6e71;">/mo</span>
			    </span>
			    <br>
			    <small class="label-show">CAMPAIGNS</small>
			        <br>
				        <b class="mobile-campaign-title">Campaigns</b>
				            <p><b>ALL</b> Recover Campaigns</p>
			            	<p><b>All</b> Retain Campaigns</p>
			    <span class="mobile-details">
			        <br>
			            <b class="mobile-campaign-title">Features</b>
				            <p><b>ALL</b> Recover Features<p>
				            <p><b>ALL</b> Retain Features</p>
				            <p class="label-plus">PLUS</p>
				            <p>A/B Testing</p>
				            <p>Hold Out Testing</p>
			        <br>
			    </span>
			</div><!-- end optimize plan -->
		</div>
		<!-- end first row -->


		<!-- spacer -->
			<div id="spacerBar" style="height:31px;">&nbsp;</div>
		<!-- end spacer -->


		<!-- /////////////////////////////
		START OF 2ND ROW 
		////////////////////////////////-->
			<div class="row">
			    <div id="allPackagesBar" class="nine columns">
			    	<p>All packages include unlimited email sends</p>
			    </div>
			</div>
		<!-- END OF 2ND ROW -->


		<!-- /////////////////////////// 
		START OF 3RD ROW 
		//////////////////////////////-->
			<div class="features-label">
				<h2>Features</h2>
			</div>
			<div id="pricingCol2" class="row">
			    <div id="recoverPlanBottom" class="three columns pricing-col">
			        <p>Email Analytics<p>
			        <p>Funnel Tracking</p>
			        <p>Pre-Submit Capture</p>
			        <p>Custom Email Templates</p>
			        <p>Frequency Capping</p>
			    </div>
			    <div id="retainPlanBottom" class="three columns pricing-col">
			        <p><b>All</b> Recover Features<p>
			        <p class="label-plus">PLUS</p>
				        <p>Retention Triggers</p>
				        <p>&nbsp;</p>
				        <p>&nbsp;</p>
				        <p>&nbsp;</p>
			    </div>
			    <div id="optimizePlanBottom" class="three columns pricing-col">
			        <p><b>ALL</b> Recover Features<p>
			        <p><b>ALL</b> Retain Features</p>
			        <p class="label-plus">PLUS</p>
			        	<p>A/B Testing</p>
			          	<p>Hold Out Testing</p>
			          	<p>Segmentation</p>
			    </div>
			</div><!-- end of pricing col -->
		<!-- END OF 3RD ROW-->

		<!-- spacer -->
			<div id="spacerBar">&nbsp;</div>
		<!-- end of spacer -->


		<!-- /////////////////////////// 
		START OF 4TH ROW 
		//////////////////////////////-->
			<div class="row" style="">
			    <div id="allPackagesBar" class="nine columns">
			    	<p>All Packages Include Full Service Account Management</p>
			    </div>
			</div> 
		<!-- END OF 4TH ROW -->
	
		<!-- start spacer -->
			<div id="spacerBar" class="btm-spacer">&nbsp;</div>
		<!-- end spacer -->

		<!-- /////////////////////////// 
		START OF 5TH ROW 
		//////////////////////////////-->
		<div id="pricingColLast" class="row">
		    <div id="bottomNote" class="nine columns">
		      <p>Unlimited Email Design, HTML/CSS Development, Campaign Strategy, Monthly Reviews</p>
		    </div>
		</div>
		<!-- END OF 5TH ROW -->
	
	</div> <!-- END OF MAJOR ROW -->
